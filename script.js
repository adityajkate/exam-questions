...
function parseQuestions(text) {
    ...
    for (const line of lines) {
        ...
        if (/^UNIT\s+\d+/i.test(line)) {
            ...
            // Highlight years
            qText = qText.replace(/(May|Dec)\s+20\d{2}/gi, match => `<span class="year-badge">${match}</span>`);
...
function showSubject(code) {
    ...
    const units = parseQuestions(data.content);
    ...
    // Build view
    let html = `
        <button class="back-button fade-in-up" stroke-delay="0.1s">← Back to Subjects</button>
        ...
        if (unit.questions5.length > 0) {
            ...
            unit.questions5.forEach((q, idx) => {
                html += `<div class="question">
                    ...
                    ${q.diagram ? `<span class="diagram-note">[Diagram]</span>` : ''}
                </div>`;
            });
            html += `</div>`;
        }
