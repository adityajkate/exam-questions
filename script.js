...
const diagramsMap = {
  ...
  ml_UNIT6_Q3_10M: ["Diagrams/ML/UNIT6_Q3_10M.png"],
  sepm_UNIT6_Q4_10M: ["Diagrams/SEPM/UNIT6_Q4_10M.png"],
};
...
function parseQuestions(text, codeStr) {
  ...
  for (const line of lines) {
    ...
    if (/^UNIT\s+\d+/i.test(line)) {
      ...
      qText = qText
        .replace(/[\(\[\-,\.\s]*(May|Dec)\s+20\d{2}[\)\],\.\s]*/gi, (match) => {
...
function buildImagesHtml(images) {
  ...
  return images
    .map(
      (src) => `<img src="${src}" class="question-diagram" loading="lazy" />`,
    )
    .join("");
}
...
function showSubject(code) {
  ...
  questionsView
    ...
    .addEventListener("click", hideSubject);
}
